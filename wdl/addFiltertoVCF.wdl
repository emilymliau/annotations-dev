...
task AddFilterToVCFShard {
    ...
    command <<<
        set -e
        bcftools annotate --header-lines ${missing_filters} -o updated.vcf ${vcf_shard}
    >>>

    output {
        File updated_vcf = basename(vcf_shard, '.vcf') + '.vcf_new.bgz'
        File updated_vcf_idx = updated_vcf + '.tbi'
    }
}